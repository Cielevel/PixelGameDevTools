...
| `style_kit.py` | **风格基准库**（通用参数化惯例）：轮廓偏移法明暗带、穹顶剪影、高光团、眼神光等；新资产优先复用只改几何与配色参数；首个风格基准资产定案后，把明暗造型参数沉淀回此库 |
| `gen_*.py` | 各资产的生成脚本（可复现、可批量改色改参）；按下方骨架与约定自建 |
...
4. **一源双产出**（可选模式）：同一像素可双写正式档（经 `layout.py` 前缀映射落 `assets/sprites/<子目录>/`）与基准存档 `base/`
...
pal = Palette.load("assets/palettes/<调色板名>.json").roles
...
完整范例（调色板先入板、一源双产出、sheet 拼装、10 帧 squash & stretch 无缝循环）：参照本目录骨架实现，首个资产定案后回流沉淀到 `style_kit.py`。
