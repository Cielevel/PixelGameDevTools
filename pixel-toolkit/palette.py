...
def gpl_text(colors, name="palette", columns=16):
    """GIMP `.gpl` 调色板文本（契约 §4.2 判据 5：调色板随资产交付，供引擎侧受击闪白/染色/换皮）。"""
    lines = ["GIMP Palette", "Name: {}".format(name), "Columns: {}".format(columns), "#"]
    for r, g, b in colors:
        lines.append("{:3d} {:3d} {:3d}\t# {:02x}{:02x}{:02x}".format(r, g, b, r, g, b))
    return "\n".join(lines) + "\n"


def export_palette(colors, path, name=None):
    """调色板落盘：按扩展名分派 `.gpl`（GIMP）/ 其他（本仓 JSON，`Palette.save` 格式）。

    颜色去重后按 RGB 数值排序——输出确定性，重跑逐字节一致。
    """
    uniq = sorted({tuple(c)[:3] for c in colors})
    name = name or os.path.splitext(os.path.basename(path))[0]
    if path.lower().endswith(".gpl"):
        with open(path, "w", encoding="utf-8") as f:
            f.write(gpl_text(uniq, name=name))
    else:
        Palette(name=name, colors=uniq).save(path)
    return path, uniq
