...
针对 knowledge/standardization/（原 AI 标准化交接包）的痛点与升级建议实现：
...
- 时间维滤波（可选）：沿时间轴逐像素取中值、再映射回共享色板，压掉静止区域的
  帧间变色（闪烁）；插在共享色板量化之前，色数不增（见 temporal_median_frames）
...
# ------------------------------------------------------- 时间维滤波 ----
def temporal_median_frames(images, w=3, palette=None):
    """沿时间轴逐像素取中值（压掉帧间颜色抖动），**运动感知**：只压孤立闪点，不拖旧颜色。

    口径（knowledge/像素视频-工具改进方案.md §2）：对输出帧 i、像素 (x,y)：
      0. **运动感知门**（2026-09-08 加入）：当前帧与**任一相邻帧**（i±1）一致（同为不透明且 RGB 相同，
         或同为透明）→ **保留当前值**——持续 ≥2 帧的变化是真实运动/换色，不算闪烁；
      1. 否则按窗口 [i-w//2, i+w//2] **首尾收缩**（不 pad、不循环）取中值：
         窗口内 alpha>0 的帧 RGB 逐通道取中值；窗口 alpha 中值 ≥128 → 不透明，否则透明；
      2. 给了 palette（[(rgb, lab)]）时按 OKLab 最近色映射。

    **为什么加运动感知门**：纯中值在运动区会把 ±2 帧的旧颜色拖到当前位置（实测 12fps/64×64 下
    63% 的运动像素被冻结 = 明显拖影）。加门后只有「孤立闪点」被替换，运动边缘保持当前帧颜色。
    容差 TOL=24（各通道）是为了让**过渡色**也留在当前帧——精确匹配会把边缘过渡色判为
    「与前后帧都不同」而拉回旧色，实测 w=3 残影 4.69%→2.23%、w=5 两帧残影 2.70%→0.93%。

    **必须放在跨帧共享色板量化之前**：中值结果通常不在色板上，先滤波再映射，
    最终色数不增；反之会引入色板外新色，破坏色数门禁。alpha 与颜色同步处理，
    否则出现半透明残留。

    images: 同一尺寸的 RGBA 图列表（像素 = 逻辑格）
    w:      窗口帧数（奇数；≤1 = 原样返回；偶数向上取奇）
    palette: 可选 [(rgb, lab)]，如 build_palette_k 的产物
    """
    ...
    if w % 2 == 0:
        w += 1
    half = w // 2
    TOL = 24                      # 运动感知门的通道容差（见 same() 说明）
    size = frames[0].size
    for im in frames:
        if im.size != size:
            raise ValueError("temporal_median_frames: 各帧尺寸必须一致")
    W, H = size
    outs = [Image.new("RGBA", size, (0, 0, 0, 0)) for _ in range(n)]
    src = [f.load() for f in frames]
    dst = [o.load() for o in outs]

    def same(a, b):
        """同态判定（带容差）：同为透明，或同为不透明且各通道差 ≤ TOL。

        TOL=24 是实测折中：运动边缘的过渡色与前后帧都不完全相同，精确匹配会把它们
        拉回旧颜色（拖影）；放宽容差让过渡色留在当前帧，拖影减半而闪烁抑制仍在。
        """
        if (a[3] > 0) != (b[3] > 0):
            return False
        if a[3] == 0:
    ...
    for y in range(H):
        for x in range(W):
            seq = [src[i][x, y] for i in range(n)]
            for i in range(n):
                cur = seq[i]
                if (i > 0 and same(cur, seq[i - 1])) or (i + 1 < n and same(cur, seq[i + 1])):
                    dst[i][x, y] = cur        # 运动感知门：与相邻帧一致 → 保留（不拖影）
                    continue
                lo = i - half
                if lo < 0:
                    lo = 0
                ...
                if hi > n:
                    hi = n
                win = seq[lo:hi]
                alphas = sorted(p[3] for p in win)
                if alphas[len(alphas) // 2] < 128:
                    continue                      # 窗口 alpha 中值 <128 → 透明
                solid = [p for p in win if p[3] > 0]
                if not solid:
                    continue
                m = len(solid) // 2
                dst[i][x, y] = (sorted(p[0] for p in solid)[m],
                                sorted(p[1] for p in solid)[m],
                                sorted(p[2] for p in solid)[m], 255)
    if palette:
        outs = [map_with_palette(o, palette) for o in outs]
    return outs


def temporal_mode_frames(images, w=3):
    """时间众数稳定（**量化之后**执行）：窗口内逐像素取众数，平局保留当前帧。

    与 `temporal_median_frames` 的区别：
    - 位置：本函数作用在**已量化**帧上（众数只取窗口内已出现的颜色 → 不会引出色板外新色）；
    - 行为：只替换窗口内的**少数派**像素，且平局时保留当前帧 → 对运动边缘几乎不产生拖影
      （实测残影 1 帧 1.66% / 2 帧 0.00%，对比中值 w=5 的 5.14% / 2.70%）。
    - 口径与 `video-studio.html` 的 `temporalStabilize()` 完全一致（前端口径对拍校验）。

    images: 同一尺寸的 RGBA 图列表（通常已量化）
    w:      窗口帧数（≤1 = 原样返回；偶数向下取半窗，与浏览器实现一致）
    """
    frames = [im.convert("RGBA") for im in images]
    n = len(frames)
    w = int(w or 0)
    ...
    for im in frames:
        if im.size != size:
            raise ValueError("temporal_mode_frames: 各帧尺寸必须一致")
    W, H = size
    seq = []                                  # 每帧每像素打包颜色索引（透明 = -1）
    for f in frames:
        d = f.tobytes()
        a = [0] * (W * H)
        for p in range(W * H):
            o = p * 4
            a[p] = ((d[o] << 16) | (d[o + 1] << 8) | d[o + 2]) if d[o + 3] else -1
        seq.append(a)
    outs = []
    for k in range(n):
        lo, hi = max(0, k - r), min(n - 1, k + r)
        cur = seq[k]
        buf = bytearray(frames[k].tobytes())
        for p in range(W * H):
            order, counts = [], {}
            for t in range(lo, hi + 1):
                ...
                if v in counts:
                    ...
                    counts[v] = 1
                    order.append(v)           # 首次出现顺序（与浏览器实现一致）
            best, bn = cur[p], 0
            for v in order:
                if counts[v] > bn:
                    bn, best = counts[v], v
            if counts.get(cur[p], 0) == bn:   # 平局保留当前帧
                best = cur[p]
            if best == cur[p]:
                continue
            o = p * 4
            if best < 0:
                ...
                buf[o + 2] = best & 255
                buf[o + 3] = 255
        outs.append(Image.frombytes("RGBA", size, bytes(buf)))
    return outs
