...
| `video-studio.html` | **视频像素化交互工作台**（纯静态单 HTML，双击即用、离线、零依赖）：① 多轨道时间轴裁剪（拖拽区块/边缘直观调整、轨道可重叠、播放头逐帧预览、每轨道独立播放/导出） ② 自定义采样帧率（12/24/30/60+custom） ③ 标准化参数（尺寸/等比/色数/描边/幕色抠像） ④ 多导出方案（HTML 播放器 / GIF / Sprite Sheet / PNG 序列 / ZIP 打包，按选中轨道导出）。处理管线与 `video.py` 同构（幕色抠像＋边缘连通清除→等比降采样→跨帧共享 OKLab 量化→描边） |
