...
> 实验功能：把**法线贴图 + 动态光照**引入 2D/像素画素材。目标形态为纯静态单 HTML（双击即用、离线、零构建零依赖）。当前处于**原型阶段**，独立演化，不构成稳定承诺。
...
- 2026-09-08：**P2 批处理完成**（v0.2.0）。GIF（ImageDecoder 逐帧）/ 图片多选 / 视频（12fps 抽帧，上限 120 帧）→ 帧条缩略图点击送入槽位阅览（‹/› 帧）→「批量转换」按当前源（高度亮度 / Alpha 距离场）逐帧出法线 →「导出法线 ZIP」。转换核心抽成纯函数 `heightToNormal` / `sourceHeight`，与实时预览共用参数。headless 端到端实测：3 图 → 3/3 法线、ZIP 三条目 CRC 通过、两条源路径均通过。
