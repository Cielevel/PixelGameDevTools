...
| 样本 | 内容 | 源唯一色 | 真值唯一色 |
|---|---|---|---|
| `samples/sampleA_f0.jpg` | 良性：纯 JPEG 噪声 | 131 | 7 |
| `samples/sampleB_f2_hard.jpg` | 困难：+高斯模糊 0.6（模拟 AA/mixels） | 946 | 8 |
| `samples/anim/f{0,2,3,6}.jpg` | 动画 4 帧（跨帧共享网格/色板测试） | ~131 | 7-9 |
...
完整数据见 `../../experimental/standardization-bench/results.json`（2026-09-08 整理：`lab/` 已迁至 `experimental/standardization-bench/`）。
...
# ③ 工装（现位于 experimental/standardization-bench/；make_samples 依赖已删脚本，见其 README）
cd experimental/standardization-bench && python3 bench.py && python3 bench_anim.py
```
